...
function CustomInputAppItem(props: CustomInputAppItemProps) {
  ...
  const searchButton = (
    <div style={{ opacity: 0.4 }}>
      <InputButton svg={searchButtonSvg} clickHandler={onClickSearchButton} />
    </div>
  );
  const removeButton = (
    <InputButton svg={removeButtonSvg} clickHandler={onClickRemoveButton} />
  );

  const button = value ? removeButton : searchButton;
  ...
  return (
    <CustomInput
      ...
      {...props}
      buttons={button}
      customClassname={"custom-input__wrapper__link"}
      isInvalid={isInvalid}
      readOnly
    />
  );
